from keras.layers import Activation, BatchNormalization, Conv2D, Dense, Input, add
...
def conv_bn_relu_block(
    ...
    def f(inputs):
        ...
        return (
            Activation("relu", name="{}_relu".format(name))(batch_norm)
            if activation
            else batch_norm
        )
...
def residual_block(block_num, **args):
    def f(inputs):
        res = conv_bn_relu_block(
            name="residual_1_{}".format(block_num), activation=True, **args
        )(inputs)
        res = conv_bn_relu_block(
            name="residual_2_{}".format(block_num), activation=False, **args
        )(res)
