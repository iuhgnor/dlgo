...
from dlgo import goboard, scoring
...
class TerminationStrategy:
    ...
    def should_pass(self, game_state: goboard.GameState) -> bool:
        return False

    def should_resign(self, game_state: goboard.GameState) -> bool:
        return False
...
class ResignLargeMargin(TerminationStrategy):
    def __init__(self, own_color: Player, cut_off_move: int, margin: float):
    ...
    def should_resign(self, game_state: goboard.GameState) -> bool:
        ...
        if self.moves_played:
            ...
            if (
                game_result.winner != self.own_color
                and game_result.winning_margin >= self.margin
            ):
                return True
        return False
...
class TerminationAgent(Agent):
    def __init__(self, agent: Agent, strategy: TerminationStrategy | None = None):
        Agent.__init__(self)
        self.agent = agent
...
# tag::get_termination[]
def get(termination: str) -> TerminationStrategy:
