...
class GoString:  # <1>
    ...
    def merged_with(self, go_string: "GoString") -> "GoString":  # <2>
...
class Board:  # <1>
    ...
    def get(self, point: Point) -> Player | None:  # <1>
        string = self._grid.get(point)
        if string is None:
            return None
        return string.color

    def get_go_string(self, point) -> GoString | None:  # <2>
        string = self._grid.get(point)
        if string is None:
            return None
        return string
...
class Move:  # <1>
    def __init__(
        self, point: Point | None = None, is_pass: bool = False, is_resign: bool = False
    ):
    ...
    @classmethod
    def play(cls, point: Point) -> "Move":  # <2>
        return Move(point=point)

    @classmethod
    def pass_turn(cls) -> "Move":  # <3>
        return Move(is_pass=True)

    @classmethod
    def resign(cls) -> "Move":  # <4>
...
class GameState:
    ...
    def apply_move(self, move: Move) -> "GameState":  # <1>
        if move.is_play:
